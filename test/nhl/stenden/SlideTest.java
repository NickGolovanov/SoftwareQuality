...
class SlideTest
{
    ...
    void testConstructor()
    {
        ...
        assertNotNull(newSlide.getTitle());
    ...
    void testGetSlides()
    {
        ...
        List<SlideItem> slides = slide.getSlides();
        assertEquals(2, slides.size());
        assertEquals(textItem, slides.get(0));
        assertEquals(bitmapItem, slides.get(1));
    }
    ...
    void testDrawWithEmptySlide()
    {
        ...
        // Verify that draw was not called for content items
    ...
    void testDrawWithDifferentAreas()
    {
        ...
        for (Rectangle area : areas)
        {
            ...
            clearInvocations(textItem, bitmapItem);
        }
    }
    ...
    void testGetScale()
    {
        ...
        for (Rectangle area : areas)
        {
            // Create a new slide for each test to avoid state from previous iterations
            slide = new Slide();
            slide.setTitle(TEST_TITLE);
            ...
            // Reset mock and add it to the new slide
            clearInvocations(textItem);
            slide.append(textItem);

            slide.draw(graphics2D, area, observer);

            // Verify that draw was called with the correct scale for both title and content
            verify(textItem, times(1)).draw(
                    eq(area.x),
                    anyInt(),
                    eq(expectedScale),
                    eq(graphics2D),
